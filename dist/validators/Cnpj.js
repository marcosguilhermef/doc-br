...
class CnpjValidator extends Validators_1.Validators {
    /**
    ...
    run() {
        ...
        const numbers = this.parameter.replaceAll(/[.,/-]/g, '').split('').map(e => parseInt(e));
    ...
    sum(x) {
        return x.slice().reverse().reduce((e, i, a) => {
            if (a > 7) {
                return (a - 6) * i + e;
            }
            return (a + 2) * i + e;
